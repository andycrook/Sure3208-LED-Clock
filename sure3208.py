...
class sure3208:
    ...
    def brightness(self,value):
        if value == 1:
            self.send_CMD(self.PWM_DUTY_1)
        if value == 2:
            self.send_CMD(self.PWM_DUTY_2)
        if value == 3:
            self.send_CMD(self.PWM_DUTY_3)
        if value == 4:
            self.send_CMD(self.PWM_DUTY_4)
        if value == 5:
            self.send_CMD(self.PWM_DUTY_5)
        if value == 6:
            self.send_CMD(self.PWM_DUTY_6)
        if value == 7:
            self.send_CMD(self.PWM_DUTY_7)
        if value == 8:
            self.send_CMD(self.PWM_DUTY_8)
        if value == 9:
            self.send_CMD(self.PWM_DUTY_9)
        if value == 10:
            self.send_CMD(self.PWM_DUTY_10)
        if value == 11:
            self.send_CMD(self.PWM_DUTY_11)
        if value == 12:
            self.send_CMD(self.PWM_DUTY_12)
        if value == 13:
            self.send_CMD(self.PWM_DUTY_13)
        if value == 14:
            self.send_CMD(self.PWM_DUTY_14)
        if value == 15:
            self.send_CMD(self.PWM_DUTY_15)
        if value == 16:
            self.send_CMD(self.PWM_DUTY_16)
        ...
        if len(bytes_val) > (self.num_screens * 32):
            ...
            if self.justify == 0:
                ...
                for byt in self.bytes_buffer:
                    ...
                    scroll = scroll + 1
            if self.justify == 1:
                scroll = int(((self.num_screens * 32) - len(bytes_val)) / 2)
                for byt in self.bytes_buffer:
                    new_b[scroll] = byt
                    scroll = scroll + 1
            if self.justify == 2:
                scroll = int((self.num_screens * 32) - len(bytes_val))
                for byt in self.bytes_buffer:
                    new_b[scroll] = byt
                    scroll = scroll + 1
            if self.justify == 3:
        ...
        for (
            ...
            if (
                ...
                if screen < self.num_screens:  # only send if the matrix is in range
                    self.send_data(
                        CMD, screen, 266
                    )  # send the CMD and data to the screen - 266 bits in all for a full matrix
